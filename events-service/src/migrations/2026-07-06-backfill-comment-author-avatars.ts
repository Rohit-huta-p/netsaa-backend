/**
 * Data backfill · 2026-07-06 · Refresh event-comment author avatars
 *
 * Older event comments stored authorImageUrl from the JWT's login-time
 ...
 * every EventComment from the author's CURRENT User record.
 ...
 * REQUIRES: EVENTS_MONGO_URI (or MONGO_URI) — loaded from .env.
 */
...
async function main() {
  ...
  log('✓ Connected');
  try {
    const comments: any[] = await EventComment.find({})
      .select('authorId authorName authorImageUrl')
      .lean();
    log(`Found ${comments.length} event comments`);
    ...
    if (DRY_RUN) {
      ...
      const res = await EventComment.bulkWrite(ops, { ordered: false });

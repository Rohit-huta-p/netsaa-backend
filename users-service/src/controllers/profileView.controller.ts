...
export const recordProfileView = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (result?.upsertedCount && result.upsertedCount > 0) {
      ...
      // NOTE: emitting a "profile.viewed" notification to the owner is deferred
      // to the notifications-v2 feature (emitProfileViewed + profile.viewed
      // type/factory), which ships separately. Recording the view and bumping
      // the lifetime counter above are all the KPI "profile views" tile needs.
    }
